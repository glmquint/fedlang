...
tmux send-keys "docker run -v .:/app/fedlang -it --rm --network=fednet --ip=172.19.0.2 fedlang bash -c 'pipenv run sh run_director.sh'" C-m
sleep 2

for i in `seq 0 $[$1-1]`; do
  # Split the window horizontally and run the client container
  tmux split-window -h
  tmux send-keys "docker run -v .:/app/fedlang -it --rm --network=fednet -e FL_CLIENT_ID=$i fedlang bash -c 'pipenv run sh start_client.sh'" C-m
  sleep 0.1
done
sleep 2
...
tmux split-window -v
tmux send-keys "docker run -v .:/app/fedlang -it --rm --network=fednet -e FL_DIRECTOR_NAME=director@172.19.0.2 -e FL_NUMBER_CLIENTS=$1 -e FL_LANGUAGE=$2 fedlang ./start_stats_node.sh" C-m
