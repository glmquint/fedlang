...
RUNTIME_FILE_PATH: str = os.environ.get("RUNTIME_FILE")
logger_info(f'RUNTIME_FILE_PATH = {RUNTIME_FILE_PATH}')
...
class FEMNISTServer(FedLangProcess):

    def __init__(self, **kwargs):
        super().__init__(**kwargs)

    def init_server(self, experiment, config, bb):
        logger_info(f'inside init_server async execution')
        config = json.loads(config)
        bb = bytes(bb, encoding='utf8')
        client_ids = list(bb)
        logger_info(
            f"experiment = {experiment}, config_file = {config}, client_ids = {client_ids}")
        ...
        experiment_configuration = config.get("parameters")
        client_configuration_str = json.dumps(experiment_configuration)
...
if __name__ == "__main__":
    ...
    gc.collect()
    start_process(FEMNISTServer, pyrlang_node_id, erl_cookie, erl_client_name, erl_worker_mailbox)
