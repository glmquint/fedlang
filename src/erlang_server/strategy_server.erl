...
init_strategy_server(DirectorPID, ExperimentID, Clients, ExperimentDataDescriptor, StatsNodePID) ->
    ...
    %MYIP = os:getenv("MYIP"),
        ...
        ClientsFLandPyerlPIDS = [receive { fl_worker_ready, ClientPID, FLPID, PyerlangPID } -> {ClientID, FLPID, PyerlangPID} end || {ClientID, ClientPID} <- Clients],
        io:format("ClientsFLandPyerlPIDS: ~p ~n", [ClientsFLandPyerlPIDS]),
        lists:map(fun({_, _, PyerlangPID}) -> PyerlangPID ! { self(), update_graph, ClientsFLandPyerlPIDS} end, ClientsFLandPyerlPIDS),
        % [receive graph_updated -> ok end || {_, _, _} <- ClientsFLandPyerlPIDS],
        ClientsFLPIDs = [{ClientID, FLPID} || {ClientID, FLPID, _} <- ClientsFLandPyerlPIDS],
        ClientsPyerlPIDs = [{ClientID, PyerlangPID} || {ClientID, _, PyerlangPID} <- ClientsFLandPyerlPIDS],
